(function () {
  ...
  function authInterceptor($log, $q, $rootScope, store, toaster) {
    ...
    function request(config) {
      ...
      //}
      if (angular.isObject(currentUser) && angular.isString(currentUser.user)) {
        config.headers.Authorization = 'Bearer ' + currentUser.user;
      }
      return config;
    }
    ...
    function responseError(reason) {
      switch (reason.status) {
        case -1:
          ...
          break;
        case 401:
          //It should be uncommented when back-end will get normalized.
